...
# 1：数据读取
train_data = pd.read_csv('./input/test_treb_month_78.csv')


test_data = pd.read_csv('./input/treb_test_month_9.csv')
...
origin_data = test_data.reset_index(drop=True)
...
preds_dataframe = pd.DataFrame(preds_Series,columns=['predictions'])

merge_data = pd.concat((origin_data,preds_dataframe),axis=1)
merge_data.to_csv('./merge_data_bak/merge_data_auto_ml_xgboost.csv')
