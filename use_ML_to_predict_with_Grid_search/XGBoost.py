...
# 寻找超参数
params = {"n_estimators":[x for x in range(1,1000,10)],
          'max_depth':[x for x in range(1,10,1)],
          'learning_rate':[x for x in np.arange(0.001,0.1,0.001)],
          # 'gamma':[x for x in np.arange(0.1,0.3,0.1)],
          # 'min_child_weight':[x for x in range(1,10,1)],
          #   'subsample':[x for x in np.arange(0.5,1,0.1)],
          #   'colsample_bytree':[x for x in np.arange(0.5,1,0.1)],
          # 'eta':[x for x in np.arange(0.1,1,0.1)]
          }
