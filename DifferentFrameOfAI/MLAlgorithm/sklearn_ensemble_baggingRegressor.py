...
@file: sklearn_ensemble_baggingRegressor.py
@time: 2018/7/31
"""
import numpy as np
...
# 使用Bagging之前要有一个若分类器，这里就用之前的ridge
...
for param in params:
    ...
    test_score = np.sqrt(-cross_val_score(clf,train,train_label,cv=10,scoring='neg_mean_squared_error'))
    test_scores.append(np.mean(test_score))

plt.plot(params,test_scores)
plt.title('bagging alpha vs Error')
plt.show()
...
plt.title("bagging pre and label distribute circumstance")
plt.legend()
plt.show()
