...
dir_path = os.path.dirname(root_path + "first_reporter_task_one_week_finish/month_2to7_delisting_data_prediction/savemodel_mydnn/")
...
def normalization_price(data):
    ...
    data_price= np.array(data[['longitude','price','latitude','bedrooms','bathroomTotal']])
    data[['longitude', 'price', 'latitude','bedrooms','bathroomTotal']] = minmax.fit_transform(data_price)
    return data
...
# 加载训练数据
data = pd.read_csv('./input/month_2to7_delistingDate_train_after_process_2.csv')
...
standard_label = StandardScaler()
label = standard_label.fit_transform(np.array(label).reshape(-1,1))
print(label)
...
def train_input_fn(is_shuffe=True,batch_size=10,num_epoch=None):
