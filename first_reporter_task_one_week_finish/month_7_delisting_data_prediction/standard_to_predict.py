...
dir_path = os.path.dirname(root_path + "first_reporter_task_one_week_finish/month_7_delisting_data_prediction/savemodel_my_dnn_standard/")
...
def train_and_validation_function():
    ...
    for period in range(periods):
        ...
        # training_predictions = minmax.inverse_transform(training_predictions.reshape(-1,1))
        ...
        predictions.to_csv('./validation_predict_result_standard.csv', index=False)
        merge_data = pd.concat((origin_data, predictions), axis=1)

        merge_data_df = pd.DataFrame(merge_data)
        merge_data_df.to_csv('./validation_merge_result_standard.csv', index=False)
