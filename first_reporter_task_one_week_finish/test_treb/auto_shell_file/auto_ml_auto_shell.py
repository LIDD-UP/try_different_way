...
def test_data_save_and_merge_data(prediction_result,origin_data,f):
    ...
    merge_data_df.to_csv('./intermediate_generated_file/auto_ml_merge_test_data.csv', index=False)
...
# 合并训练数据预测结果和原始数据，以方便做数据的去除，一遍下一次进行切分；
def train_data_save_and_merge(model,origin_data_train,X_train_data,f):
    ...
    merge_train_data_df.to_csv('./intermediate_generated_file/auto_ml_merge_train_data.csv', index=False)
    f.write('finish train data prediction and save the merge the train data and label\n')


# 计算训练数据的预测结果
def process_train_merge_data_remove_some_data(f,remove_ratio):

    data = pd.read_csv('./intermediate_generated_file/auto_ml_merge_train_data.csv')
    # 用于保存上一次的文件；
    data_orgin = data
    ...
    # print(data.head())
    print(data.shape)
    data.to_csv('./input/treb_toronto_3to8_auto_ml.csv')
    # 用于保存上一次的文件；
    data_orgin.to_csv('./intermediate_generated_file/auto_ml_last_train_data.csv')
...
if __name__ == '__main__':
    # 由于程序问题，在保存成csv文件的时候可能会相对较慢，
    # 如果这时候再进行读取文件可能都不到，所以需要设置一个延迟时间，
    ...
    is_first_train = 1
    # 指定log文件目录；
    log_path = './log_file/auto_ml_log.log'
    ...
    for i in [300,
        ...
        with open(log_path,'a+',encoding='utf8') as f:
            ...
            '''
             训练数据读取：
             判断是否时第一次训练，如果是就需要读取原始的文件；
             不是则需要读取中间生成的文件
            '''
            if is_first_train:
                df_train = pd.read_csv('./input/treb_toronto_3to8.csv')
            else:
                df_train = pd.read_csv('./input/treb_toronto_3to8_auto_ml.csv')
            ...
            print(mean_absolute_error(df_test_label,pred1))
            # 计算比例：
            # 合并保存测试数据预测结果
            test_data_save_and_merge_data(pred1,origin_data,f)
            # 合并保存训练数据的预测结果
            train_data_save_and_merge(ml_predictor,origin_data_train,df_train_prediction,f)
            time.sleep(delay_time)
            # 开始切分数据
            process_train_merge_data_remove_some_data(f,i)
        is_first_train=0
