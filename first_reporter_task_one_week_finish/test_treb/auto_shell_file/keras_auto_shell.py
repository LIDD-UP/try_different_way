...
# keras剩下的训练时基于之前的训练的，所以只需要导入模型即可
...
def test_data_save_and_merge_data(prediction_result,origin_data,f):
    ...
    merge_data_df.to_csv('./intermediate_generated_file/keras_merge_test_data.csv', index=False)
...
def train_data_save_and_merge(model,origin_data_train,X_train_data,f):
    ...
    merge_train_data_df.to_csv('./intermediate_generated_file/keras_merge_train_data.csv', index=False)
...
def process_train_merge_data_remove_some_data(f,remove_ratio):
    ...
    data = pd.read_csv('./intermediate_generated_file/keras_merge_train_data.csv')
    # 用于保存上一次的文件；
    data_orgin = data
    ...
    # print(data.head())
    print(data.shape)
    data.to_csv('./input/treb_toronto_3to8_keras.csv')
    # 用于保存上一次的文件；
    data_orgin.to_csv('./intermediate_generated_file/keras_last_train_data.csv')
...
if __name__ == '__main__':
    ...
    is_first_train = 1
    # 指定log文件目录；
    log_path = './log_file/keras_log.log'
    model_path = './model_file/keras_model.h5'
    ...
    for i in [300,
        ...
        with open(log_path,'a+',encoding='utf8') as f:
            ...
            if is_first_train:
                ...
                df_train = pd.read_csv('./input/treb_toronto_3to8_keras.csv')
            ...
            # 这里可能需要不重以前的模型继续进行训练；
            model = first_train(X_train_data,Y_train_data_label,100,f)
