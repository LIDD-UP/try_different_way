...
test_data_6 = pd.read_csv('test_data_6_1.csv')
test_data_6 = test_data_6.dropna()
x_test = test_data_6[['longitude','latitude', 'price', 'buildingTypeId',]]
...
train_x_disorder = x
test_x_disorder = x_test
train_y_disorder = y
test_y_disorder = y_test
...
train_y_disorder = ss_y.fit_transform(train_y_disorder.reshape(-1, 1)) # 一维的数组一般都会报错，所以需要reshape一下；
...
xs = tf.placeholder(tf.float32, [None, 4])  # 原始数据的维度：16
ys = tf.placeholder(tf.float32, [None, 1])  # 输出数据为维度：1

keep_prob = tf.placeholder(tf.float32)  # dropout的比例

x_image = tf.reshape(xs, [-1, 2, 2, 1])  # 原始数据16变成二维图片4*4
## conv1 layer ##第一卷积层
...
origin_prediction_value = ss_y.inverse_transform(prediction_value)
print(origin_prediction_value)
print('---------------')
origin_y_data = ss_y.inverse_transform(test_y_disorder)
print(test_y_disorder)
print(origin_y_data)
print(mean_absolute_error(origin_y_data,origin_prediction_value))
###画图###########################################################################
