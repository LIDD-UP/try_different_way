...
@file: dnn_to_predict_month_7_deal.py
@time: 2018/9/13
"""
...
dir_path = os.path.dirname(root_path + "report_process_data_predict/use_7_month_delisting_to_predict/savemodel_modify_net/")
...
# display.display(canada_housing_data['longitude'].describe())
# display.display((canada_housing_data['latitude']).describe())
...
def pre_process_features(canada_housing_data):
    ...
    processed_features["postalCode"] = postCodeList
...
def train_nn_regression_model(
    ...
    for period in range(0, periods):
        ...
        predict_validation = pd.DataFrame()
        predict_validation['predictions'] = validation_predictions
        predict_validation['predictions'] = round(abs(predict_validation['predictions']))
        predict_validation.to_csv('./middle.csv',index=False)
        print(predict_validation.shape)
        merge = pd.concat((origin_data, predict_validation), axis=1)
        merge_df_data = pd.DataFrame(merge)
        merge_df_data.to_csv('./merge.csv',index=False)
        # merge_data_df_province_data = merge[merge['province'].isin(['Quebec', 'Ontario'])]
