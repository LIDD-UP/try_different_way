...
import os
data = pd.read_csv('../input/half_a_year.csv',low_memory=False)
...
list = ['province','city','address','postalCode','longitude','latitude','price','buildingTypeId','tradeTypeId','listingDate','bedrooms','ownershiptype']

# print(data.head())

data = data[['province','city','address','postalCode','longitude','latitude','price','buildingTypeId','tradeTypeId','listingDate','bedrooms','ownershiptype','daysOnMarket']]
data = data.dropna()
print(data.shape)
...
def divide_daysonmarket(data,num,step):
    i = 0
    for j in range(num):
        # csv_name = 'in_{}_days'.format(j*7)
        data_new = data[(data.daysOnMarket<=j*step) & (data.daysOnMarket>=0)]
...
data = pd.read_csv('./input/base_data_no_skew_encode.csv')
...
def create_report(msg):
    base = '../in_different_days_to_predict/'
    for j in range(24):
        file_name = base + 'in_{}_days/'.format(j * 7)+'in_{}_days_dnn.py'.format(j * 7)
