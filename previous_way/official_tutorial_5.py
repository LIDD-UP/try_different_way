...
# 加载训练数据
data = pd.read_csv(dirname + train_filename, header=0, usecols=[0, 1, 2, 4, 5, 6, 8, 10, 12, 14],
                   names=['province', 'city', 'address', 'longitude', 'latitude', 'price', 'buildingTypeName',
                          'tradeTypeName', 'listingDate', 'daysOnMarket'])
...
# 加载测试数据
data_test = pd.read_csv(dirname + test_filename, header=0, usecols=[0, 1, 2, 4, 5, 6, 8, 10, 12, 14],
                        names=['province', 'city', 'address', 'longitude', 'latitude', 'price', 'buildingTypeName',
                               'tradeTypeName',  'listingDate', 'daysOnMarket'])
...
# 定义基本特征和组合特征
base_columns = [
    price_bucket,  tradeTypeName, province, city, address, buildingTypeName, year_categorical,
...
estimator_model = tf.estimator.DNNLinearCombinedRegressor(
    model_dir='./tmp_official5/predict_model',
    linear_feature_columns=base_columns + crossed_columns,
    dnn_feature_columns=deep_columns,
    # dnn_hidden_units= [2048,1024, 512, 256,128,64,32,16],
    dnn_hidden_units=[16, 32, 64, 128, 256, 512, 1024, 2048,4096],
